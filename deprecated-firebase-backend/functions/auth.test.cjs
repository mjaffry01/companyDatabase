...
test('unapproved users cannot read or write',async()=>{const f=fixture();for(const action of ['contacts','companies','addContact'])await assert.rejects(f.call({auth,data:{action,approved:true}}),{code:'permission-denied'});assert.equal(f.reads(),0);});
